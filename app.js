...
import { searchFatSecretFood, getFatSecretFoodDetails, extractNutritionPer100g, loadCSVDatabase, searchCSVFood } from './fatsecret-api.js';
...
async function fatsecretSearch() {
  ...
  try {
    ...
    // 2️⃣ 영어 입력 또는 CSV 결과 없음 → FatSecret 검색
    console.log('[FatSecret] 검색 시도:', q);
    const consumerKey = localStorage.getItem('fs_consumer_key');
    const consumerSecret = localStorage.getItem('fs_consumer_secret');

    if (!consumerKey || !consumerSecret) {
      results.innerHTML = `
        <div style="padding:12px;color:var(--diet-bad);font-size:12px;line-height:1.6">
          ⚠️ FatSecret 설정이 필요합니다.<br>
          설정 탭에서 Consumer Key와 Secret을 입력해주세요.
        </div>`;
      return;
    }

    const fatsecretFoods = await searchFatSecretFood(q);
    console.log('[FatSecret 검색 결과]', fatsecretFoods?.length || 0, '개');

    if (!fatsecretFoods || fatsecretFoods.length === 0) {
      results.innerHTML = '<div style="padding:12px;text-align:center;color:var(--muted);font-size:12px">❌ 결과 없음 (CSV & FatSecret 모두 검색됨)</div>';
      return;
    }

    // FatSecret 결과 표시
    results.innerHTML = fatsecretFoods.map((food, idx) => {
      const desc = `${food.type || 'Generic'}`;
      return `
        <div class="fs-result-row" onclick="fatsecretSelectFoodById('${idx}')" style="cursor:pointer;padding:8px;border-bottom:1px solid var(--border);transition:background 0.2s" onmouseover="this.style.background='var(--bg2)'" onmouseout="this.style.background='transparent'">
          <div class="fs-result-name" style="font-weight:500">${food.name}</div>
          <div style="font-size:10px;color:var(--muted2)">🌍 FatSecret | ${desc}</div>
        </div>`;
    }).join('');

    window._fsSearchItems = fatsecretFoods;
  } catch(e) {
...
async function fatsecretSelectFoodById(idx) {
  ...
  let nutrition = null;
  const isCsvFood = food.energy !== undefined && food.protein !== undefined;
  ...
  if (isCsvFood) {
    ...
    // 📡 FatSecret 데이터 → 상세 정보 로드
    console.log('[FatSecret 선택]', food.name);
    try {
      const details = await getFatSecretFoodDetails(food.id);
      nutrition = extractNutritionPer100g(details.food.servings);
      if (!nutrition) {
        alert('영양 정보를 찾을 수 없습니다.');
        return;
      }
    } catch(e) {
      alert(`오류: ${e.message}`);
  ...
  if (nutritionPreview) {
    const source = isCsvFood ? '🇰🇷 CSV' : '🌍 FatSecret';
